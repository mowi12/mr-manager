...
def get_default_user_config() -> UserConfig:
    ...
    return UserConfig(
        discovery_cache_ttl_seconds=DEFAULT_DISCOVERY_CACHE_TTL_SECONDS,
...
def _validate_discovery_cache_ttl_seconds(value: str | None) -> int:
    """Validate and normalize configured cache TTL in seconds."""
    if value is None or value == "":
        return DEFAULT_DISCOVERY_CACHE_TTL_SECONDS
    try:
        ttl_seconds = int(value)
    except ValueError as error:
        msg = f"Invalid discovery.cache_ttl_seconds value: {value!r}"
        raise ValueError(msg) from error
    if ttl_seconds <= 0:
        msg = "discovery.cache_ttl_seconds must be greater than 0."
        raise ValueError(msg)
    return ttl_seconds
...
def load_user_config(config_path: Path | None = None) -> UserConfig:
    ...
    return UserConfig(
        discovery_cache_ttl_seconds=_validate_discovery_cache_ttl_seconds(
            parsed_values.get("discovery.cache_ttl_seconds")
        ),
...
def save_user_config(config: UserConfig, config_path: Path | None = None) -> None:
    ...
    """
    ttl_seconds = _validate_discovery_cache_ttl_seconds(str(config.discovery_cache_ttl_seconds))
    discovery_root = _validate_discovery_root(str(config.discovery_root))
    resolved_config_path = config_path or get_user_config_path()
    config_text = (
        "discovery:\n"
        f"  cache_ttl_seconds: {ttl_seconds}\n"
